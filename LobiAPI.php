<?php
namespace LobiClientPHP\LobiAPI;

use LobiClientPHP\HttpAPI\Http;
use LobiClientPHP\HttpAPI\Header;
...
class LobiAPI{
    ...
    public function __construct(){
        require_once("HttpAPI\\Header.php");
        require_once("HttpAPI\\Http.php");
	...
	public function GetMe(){
		...
		return json_decode($this->NetworkAPI->get('https://web.lobi.co/api/me?fields=premium', $header),true);
	}
	...
	public function GetPublicGroupList(){
		...
		$index = 1;
		while(true){
			$pg = json_decode($this->NetworkAPI->get("https://web.lobi.co/api/public_groups?count=1000&page=$index&with_archived=1", $header),true);
			$index++;
	...
	public function GetPrivateGroupList(){
		...
		$index = 1;
		while(true){
			$pg = json_decode($this->NetworkAPI->get("https://web.lobi.co/api/groups?count=1000&page=$index", $header),true);
			$index++;
			if(count($pg[0]["items"]) == 0)
				break;
			foreach($pg as $pgbf)
	...
	public function GetNotifications(){
		...
		return json_decode($this->NetworkAPI->get('https://web.lobi.co/api/info/notifications?platform=any', $header),true);
	}
	...
	public function GetContacts($uid){
		...
		return json_decode($this->NetworkAPI->get("https://web.lobi.co/api/user/$uid/contacts", $header),true);
	}
	...
	public function GetFollowers($uid){
		...
		return json_decode($this->NetworkAPI->get("https://web.lobi.co/api/user/$uid/followers", $header),true);
	}
	...
	public function GetGroup($uid){
		...
		return json_decode($this->NetworkAPI->get("https://web.lobi.co/api/group/$uid?error_flavor=json2&fields=group_bookmark_info%2Capp_events_info", $header),true);
	}
	...
	public function GetGroupMembersCount($uid){
		...
		$result = json_decode($this->NetworkAPI->get("https://web.lobi.co/api/group/$uid?error_flavor=json2&fields=group_bookmark_info%2Capp_events_info", $header),true);
	...
	public function GetGroupMembers($uid){
		...
		$limit = 10000;
		while($limit-- > 0){
			$g = json_decode($this->NetworkAPI->get("https://web.lobi.co/api/group/$uid?members_cursor=$next", $header),true);
	...
	public function GetThreads($uid, $count = 20){
		...
		return json_decode($this->NetworkAPI->get("https://web.lobi.co/api/group/$uid/chats?count=$count", $header),true);
	}
